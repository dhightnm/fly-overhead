 ...
 * Run with: node server/services/testRouteDataMatch.js
 */

require('dotenv').config();
const postgresRepository = require('./repositories/PostgresRepository');
const flightPlanRouteService = require('./FlightPlanRouteService');

async function testRouteDataMatching() {
  try {
    console.log('Testing route data matching with navaids table...\n');

    // Get sample routes from database
    ...
    // Test parsing and matching for each sample
    let totalCodes = 0;
    let foundCodes = 0;
    let testResults = [];
    ...
    for (const sample of samples) {
      ...
      // Test lookup for each waypoint
      let found = 0;
      const lookupResults = [];

      for (const code of waypointCodes.slice(0, 10)) { // Test first 10
